...
library(readxl)
FPS10 <- read_excel("C:/Users/lina4064/OneDrive - Nexus365/Carbapenemase Metaanalysis/CMA-R-Projects/FPDataSup2.xlsx", 
...
forest.rma(FPS10RMA, slab = paste(FPS10$Author, FPS10$`Pub Year`), ilab = cbind(FPS10$ilab2, FPS10$ilab1), ilab.pos = 4, 
           ...
           cex = 0.65, rows = c(60:53, 47:45, 39:5), ylim = c(-3, 103), cex.lab = 0.75, top = 42)
...
text(-1.6, c(61, 48, 40), pos = 4, c("ESBL Producing", "ACBL Producing", "Naive Isolates"), font = 2, cex = 0.8)

text(-1.6, c(64.25), pos = 4, c("Enterobacteriaceae - Clinical"), font = 4, cex = 0.9)
...
text(1.9274, c(63), pos = 4, c("Weight% Pr[95% CI]"), font = 2, cex = 0.75)

RMASG1 <- rma(yi = yi, vi = vi, method = "DL", knha = TRUE, data = FPS10IR, subset = (SubGroup=="ESBL"))
RMASG3 <- rma(yi = yi, vi = vi, method = "DL", knha = TRUE, data = FPS10IR, subset = (SubGroup=="ACBL")) 
...
addpoly(RMASG3, row = 42.5, cex = 0.7, mlab = "", border = 0, col = rgb(64, 64, 64, maxColorValue = 255))
...
text(-1.6, c(42.5), bquote(paste("(",Tau^2, " = ",.(formatC(RMASG3$tau2, digits=4, format="f")), ", df = ", 
                                 .(RMASG3$k - RMASG3$p), ", ", "Q = ", .(formatC(RMASG3$QE, digits=2, format="f")), ",")), pos=4, cex=0.65)

text(-1.6, c(41.5), bquote(paste("p ", .(metafor:::.pval(RMASG3$QEp, digits=4, showeq=TRUE, sep=" ")), "; ", H^2, " = ", 
