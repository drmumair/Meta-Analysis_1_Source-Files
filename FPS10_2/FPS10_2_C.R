...
library(readxl)
FPS10_2 <- read_excel("/Users/Umair/Library/CloudStorage/OneDrive-Nexus365/Carbapenemase Metaanalysis/CMA-R-Projects/FPDataSup2.xlsx", 
...
forest.rma(FPS10_2RMA, slab = paste(FPS10_2$Author, FPS10_2$`Pub Year`), ilab = cbind(FPS10_2$ilab2, FPS10_2$ilab1), ilab.pos = 4, 
           ...
           cex = 0.65, rows = c(53:48, 42:40, 34:5), ylim = c(-3, 103), cex.lab = 0.75, top = 49) #c(56:51, 45:43, 37:5)
...
text(-1.6, c(54, 43, 35), pos = 4, c("ESBL Producing", "ACBL Producing", "Naive Isolates"), font = 2, cex = 0.8)

text(-1.6, c( 57.25 ), pos = 4, c("Enterobacteriaceae - Clinical"), font = 4, cex = 0.9)


text(-1.6, c(56), pos = 4, c("Subgroups/Author(s)"), font = 2, cex = 0.75)
text(1.545, c(57), pos = 4, c("Trait-"), font = 2, cex = 0.75)
text(1.545, c(56), pos = 4, c("Species"), font = 2, cex = 0.75)
text(1.215, c(56), pos = 4, c("Sample"), font = 2, cex = 0.75)
text(1.9274, c(56), pos = 4, c("Weight% Pr[95% CI]"), font = 2, cex = 0.75)

RMASG1 <- rma(yi = yi, vi = vi, method = "DL", knha = TRUE, data = FPS10_2IR, subset = (SubGroup=="ESBL")) 
RMASG3 <- rma(yi = yi, vi = vi, method = "DL", knha = TRUE, data = FPS10_2IR, subset = (SubGroup=="ACBL")) 
...
addpoly(RMASG3, row = 37.5, cex = 0.7, mlab = "", border = 0, col = rgb(64, 64, 64, maxColorValue = 255))
...
text(-1.6, c(37.5), bquote(paste("(",Tau^2, " = ",.(formatC(RMASG3$tau2, digits=4, format="f")), ", df = ", 
                                 .(RMASG3$k - RMASG3$p), ", ", "Q = ", .(formatC(RMASG3$QE, digits=2, format="f")), ",")), pos=4, cex=0.65)

text(-1.6, c(36.5), bquote(paste("p ", .(metafor:::.pval(RMASG3$QEp, digits=4, showeq=TRUE, sep=" ")), "; ", H^2, " = ", 
